...
namespace skyline::gpu::interconnect {
    ...
    void MaxwellDma::Copy(span<u8> dstMappings, span<u8> srcMappings) {
        auto srcBuf{gpu.buffer.FindOrCreate(srcMappings, executor.tag, [this](std::shared_ptr<Buffer> buffer, ContextLock<Buffer> &&lock) {
            executor.AttachLockedBuffer(buffer, std::move(lock));
        })};
        ContextLock srcBufLock{executor.tag, srcBuf};

        auto dstBuf{gpu.buffer.FindOrCreate(dstMappings, executor.tag, [this](std::shared_ptr<Buffer> buffer, ContextLock<Buffer> &&lock) {
            executor.AttachLockedBuffer(buffer, std::move(lock));
        })};
